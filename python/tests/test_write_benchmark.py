...
5. This holds true for dynamic, out-of-order channel writes.
"""
...
def expected_data():
    ...
    return {
        ...
        "dynamic_scenario": {
            "data": {
                "G1/OldChannel": np.array([1, 2, 98, 99], dtype=np.int32),
                "G2/NewChannel": np.array([100, 101], dtype=np.float64),
            },
            "props": {
                "file_status": "active",
                "prop_a": "final", # Final (overwritten) value
                "prop_b": "new",
            }
        }
    }
...
def written_files(expected_data):
    ...
    # --- File 4: Dynamic/Out-of-Order Writes ---
    fd, path = tempfile.mkstemp(suffix='.tdms')
    os.close(fd)
    files["dynamic"] = path

    with tdms.TdmsWriter(path) as writer:
        # Segment 1: Create and write to G1/OldChannel
        writer.create_channel("G1", "OldChannel", tdms.DataType.I32)
        writer.set_channel_property("G1", "OldChannel", "prop_a", "initial")
        writer.write_data("G1", "OldChannel", np.array([1, 2], dtype=np.int32))
        writer.flush()

        # Segment 2: Create a new group and channel, write to it
        writer.create_channel("G2", "NewChannel", tdms.DataType.F64)
        writer.set_file_property("file_status", "active")
        writer.set_channel_property("G2", "NewChannel", "prop_b", "new")
        writer.write_data("G2", "NewChannel", np.array([100, 101], dtype=np.float64))
        writer.flush()

        # Segment 3: Go back and write to the *old* channel, and update its prop
        writer.set_channel_property("G1", "OldChannel", "prop_a", "final")
        writer.write_data("G1", "OldChannel", np.array([98, 99], dtype=np.int32))
        writer.flush()
...
# --- 4. Dynamic/Out-of-Order Writes ---

def test_read_dynamic_tdms_python(written_files, expected_data):
    """Reads the 'dynamic' file with tdms-python and validates."""
    path = written_files["dynamic"]
    expected = expected_data["dynamic_scenario"]
    
    with tdms.TdmsReader(path) as reader:
        # Check data
        data_a = reader.read_data("G1", "OldChannel")
        data_b = reader.read_data("G2", "NewChannel")
        validate_channel_data(data_a, expected["data"]["G1/OldChannel"])
        validate_channel_data(data_b, expected["data"]["G2/NewChannel"])
        
        # Check properties
        assert reader.get_file_properties()["file_status"] == expected["props"]["file_status"]
        assert reader.get_channel_properties("G1", "OldChannel")["prop_a"] == expected["props"]["prop_a"]
        assert reader.get_channel_properties("G2", "NewChannel")["prop_b"] == expected["props"]["prop_b"]

def test_read_dynamic_nptdms(written_files, expected_data):
    """Reads the 'dynamic' file with nptdms and validates."""
    path = written_files["dynamic"]
    expected = expected_data["dynamic_scenario"]
    
    with TdmsFile.open(path) as file:
        # Check data
        data_a = file["G1"]["OldChannel"][:]
        data_b = file["G2"]["NewChannel"][:]
        validate_channel_data(data_a, expected["data"]["G1/OldChannel"])
        validate_channel_data(data_b, expected["data"]["G2/NewChannel"])
        
        # Check properties
        assert file.properties["file_status"] == expected["props"]["file_status"]
        assert file["G1"]["OldChannel"].properties["prop_a"] == expected["props"]["prop_a"]
        assert file["G2"]["NewChannel"].properties["prop_b"] == expected["props"]["prop_b"]